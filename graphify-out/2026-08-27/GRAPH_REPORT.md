...
Cohesion: 0.25
Nodes (7): Ampliar wine-enrich con búsqueda web (region/denominacion/descripcion) — Implementation Plan, Task 1: Tipos TypeScript, Task 2: Migración Supabase — columna `descripcion_fuente_url`, Task 3: Workflow n8n — ampliar "Vinoteca – Wine Enrich", Task 4: Frontend — `Scan.tsx` + `useWines.ts`, aplicar los campos nuevos en el merge y persistirlos, Task 5: Frontend — `WineDetail.tsx`, nota de atribución bajo `descripcion`, Task 6: Verificación end-to-end
...
Cohesion: 0.06
Nodes (42): useSommelier(), callAyudaChat(), callEnriquecimiento(), callImprovePhoto(), callMaridaje(), callScanAnalizar(), callScanIdentificar(), callSommelierChat() (+34 more)
...
Cohesion: 0.12
Nodes (15): 1. Workflow n8n — nodos nuevos entre `Preparar Grounding` y `OpenAI Enrich`, 2. `OpenAI Enrich` — prompt y JSON de salida ampliados, 3. `Validar Respuesta` — nuevos `FieldTrace`, 4. Segunda pasada extractiva (`OpenAI Extraer de Búsqueda` + `Fusionar Campos`), 5. Tipos TypeScript (`src/types/index.ts`), 6. Frontend — `Scan.tsx`, 7. Migración Supabase — nueva columna, 8. Frontend — `WineDetail.tsx` (+7 more)
